...
const PRESETS = [
  { label: "متساوية", values: [8, 8, 8, 8] },
  { label: "متدرجة", values: [4, 8, 12, 16] },
  { label: "ثلاث قيم", values: [6, 9, 12] },
];

const MAX_VALUE = 20;

export function AverageLab({ onInteract, compact }: Props) {
  const [values, setValues] = useState([4, 8, 12, 16]);
    ...
    >
      <div className="rounded-2xl bg-white/90 px-3 py-2.5 ring-1 ring-teal-100">
        <p className="text-[13px] font-bold leading-snug text-ink">
          اضغط أي قيمة لزيادتها، أو اختر مثالاً جاهزاً.
        </p>
        <p className="mt-1 text-[12px] leading-snug text-slate-500">
          راقب تغيّر المجموع والمتوسط مباشرة.
        </p>
      </div>

      <div className="mt-5 flex min-h-40 items-end justify-center gap-2" aria-label="قيم المتوسط">
      ...
      <div className="mt-3 rounded-2xl bg-teal-600 px-3 py-4 text-center text-white shadow-sm">
        <p className="text-[11px] font-bold text-teal-100">المتوسط الحسابي</p>
        <p className="mt-1 text-xl font-black tabular-nums" dir="ltr">
          {sum} ÷ {values.length} = {Number.isInteger(mean) ? mean : mean.toFixed(2)}
        </p>
      </div>
